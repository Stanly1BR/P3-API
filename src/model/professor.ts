...
export class ProfessorModel {
    ...
    async findById(id: number): Promise<Professor | null>{
        const result = await pool.query('SELECT * FROM professor WHERE id_professor = $1', [id]);
        return result.rows[0] || null;
    }

    async create(data: CreateProfessorDTO): Promise<Professor>{
        const { id_titulo, tx_nome, tx_sexo, tx_estado_civil, dt_nascimento, tx_telefone } = data;
    ...
    async update(id: number, data: Partial<CreateProfessorDTO>): Promise<Professor | null> {
        const fields = [];
        const values = [];
        let index = 1;

        if (data.id_titulo !== undefined) {
            fields.push('id_titulo = $');
    ...
    async delete(id: number): Promise<boolean> {
        const result = await pool.query('DELETE FROM professor WHERE id_professor = $1', [id]);
