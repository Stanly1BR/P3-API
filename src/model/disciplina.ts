...
export class DisciplinaModel {
  ...
  async update(
    id: number,
    data: Partial<CreateDisciplinaDTO>,
    ...
    if (data.id_tipo_disciplina !== undefined) {
      fields.push(`id_tipo_disciplina = $${index++}`);
      values.push(data.id_tipo_disciplina);
    }

    if (data.tx_sigla !== undefined) {
      fields.push(`tx_sigla = $${index++}`);
    ...
    if (data.in_periodo !== undefined) {
      fields.push(`in_periodo = $${index++}`);
      values.push(data.in_periodo);
    }

    if (data.in_carga_horaria !== undefined) {
      fields.push(`in_carga_horaria = $${index++}`);
    ...
    const result = await pool.query(
      `UPDATE disciplina SET ${fields.join(", ")} WHERE id_disciplina = $${index} RETURNING *`,
      values,
    );
